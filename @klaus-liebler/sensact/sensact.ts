...
export class cCANMessageBuilderParserOld{
    ...
    public TraceCommandMessage(m:canmonitor.NotifyCanMessage){
        ...
        var s= `ApplicationCommand (old CAN-ID) to id 0x${dc.destinationAppId.toString(16)} (${sensact.ApplicationId[dc.destinationAppId]}); command:0x${dc.commandId.toString(16)} (${sensact.Command[dc.commandId]}); len:${payloadLen}; payload: 0x${common.uint8Array2HexString(arr)}`
        console.log(s);
        return s;
