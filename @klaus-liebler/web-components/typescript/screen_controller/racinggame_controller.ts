...
export function runCarRace(c: HTMLCanvasElement) {
    // debug settings
    ...
    let randomSeed: any;                 // random seed for level
    let startRandomSeed: any;            // save the starting seed for active use
    let nextCheckPoint: number;             // distance of next checkpoint
    let hueShift: number;                   // current hue shift for all hsl colors
    let road: any;                       // the list of road segments
    let time: number;                       // time left before game over
    let lastUpdate = 0;             // time of last update
    let timeBuffer = 0;             // frame rate adjustment
    let playerPitchSpring: any;
    let playerPitchSpringVelocity: any;
    let playerPitchRoad: any
    ...
    function Update() {
        ...
        if (mouseWasPressed) {
            DrawText(Math.ceil(time = Clamp(time - timeDelta, 0, maxTime)).toString(), 9); // show and update time
            context.textAlign = 'right';                                        // set right alignment for distance
            DrawText((0 | playerPos.z / 1e3).toString(), c.width - 9);                             // show distance
        }
        else {
    ...
    const LSHA = (l: any, s = 0, h = 0, a = 1) => `hsl(${h + hueShift},${s}%,${l}%,${a})`;
    const Clamp = (v: number, min: number, max: number) => Math.min(Math.max(v, min), max);
    const ClampAngle = (a: number) => (a + Math.PI) % (2 * Math.PI) + (a + Math.PI < 0 ? Math.PI : -Math.PI);
    const Lerp = (p: number, a: number, b: number) => a + Clamp(p, 0, 1) * (b - a);
    const Random = (max = 1, min = 0) => Lerp((Math.sin(++randomSeed) + 1) * 1e5 % 1, min, max);

    // simple 3d vector class
    class Vector3 {
        constructor(public x = 0, public y = 0, public z = 0) { }
        Add(v: any) { v = isNaN(v) ? v : new Vector3(v, v, v); return new Vector3(this.x + v.x, this.y + v.y, this.z + v.z); }
        Multiply(v: any) { v = isNaN(v) ? v : new Vector3(v, v, v); return new Vector3(this.x * v.x, this.y * v.y, this.z * v.z); }
    }

    // draw a trapazoid shaped poly
    function DrawPoly(x1: number, y1: number, w1: number, x2: number, y2: number, w2: number, fillStyle: string | CanvasGradient | CanvasPattern) {
    ...
    function DrawText(text: string, posX: number) {
    ...
    if (typeof ontouchend != 'undefined') {
        let ProcessTouch = (e: any) => {
            e.preventDefault();
            mouseDown = (e.touches.length > 0) ? 0 : 1;
            mouseWasPressed = 1;
            touchMode = 1;
    ...
    let inputIsDown: any = [];
    let inputWasDown: any = [];
    let inputWasPushed: any = [];
    onkeydown = e => inputIsDown[e.keyCode] = 1;
    onkeyup = e => inputIsDown[e.keyCode] = 0;
    function UpdateInput() {
        inputWasPushed = inputIsDown.map((e: any, i: any) => e && !inputWasDown[i]);
